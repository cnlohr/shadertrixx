...
Shader "Custom/ReliefDepthWriteCatlike2"
{
    Properties
    {
		...
		CGINCLUDE
			...
			void frag(v2f i, out float4 colo:COLOR, out float deptho : DEPTH)
			{
				...
				//#define PARALLAX_NOT_RELIEF
					...
					depthDiff /= length(tvduv);

					///???!?!?!?! We have to do something here, which will decrease the value if we're right over it or increase it if we're off angle.
					//It's not too bad so we can leave this as a TODO!
					depthDiff /= _ParallaxStrength/2; //Just a fudge factor.,
				#endif
