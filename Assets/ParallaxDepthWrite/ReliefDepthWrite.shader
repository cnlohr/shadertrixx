...
Shader "Custom/ReliefDepthWrite"
{
    Properties
    {
	...
	SubShader
	{
		...
		CGINCLUDE
			...
			float2 ParallaxRaymarching (float2 uv, float3 viewDir, out float travel) {
			
				//NOTE: At glancing angles, viewDir.xy will be larger, so our "stepSize" should remain constant.
			
				float stepSize = 2.0 / PARALLAX_RAYMARCHING_STEPS;
				...
				travel = length(uvOffset.xy) / length(viewDir.xy);
				return uvOffset;
			}
			...
			void frag(v2f i, out float4 colo:COLOR, out float deptho : DEPTH)
			{
				...
				tvd.xy = tvd.xy / (tvd.z + .42);
				
				//Do this for parallax mapping.
				//float offset = tex2D( _DisplacementMap, uv ) - 0.1;
				//uv += tvd.xy * _ParallaxStrength * offset;
				uv += ParallaxRaymarching( uv, tvd.xyz, depthDiff );
				depthDiff *= -12. * tvd.z;
				if( 0 && frac(_Time.y*5.)<0.8 )
				{
					colo = float4( depthDiff.xxx, 1. );
					return;
				}
				//depthDiff = -1.0;
				//depthDiff = 0.0;
