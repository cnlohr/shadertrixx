...
public class AudioLink2 : UdonSharpBehaviour
#else
public class AudioLink2 : MonoBehaviour
#endif
{
    ...
    public float[] audioThresholds = {0.75f, 0.75f, 0.75f, 0.75f};
    ...
    private void Update() 
    {
        ...
        audioMaterial.SetFloatArray("_Chunks", _chunks);
        #endif
    }
    ...
    {
        _lut = new float[audioBands.Length];
        _chunks = new float[audioBands.Length];
        for(var i=0; i<audioBands.Length; i++)
        {
            //audioBands[i] = Mathf.Clamp(audioBands[i], 20f, 20000f);
            float bandStart = audioBands[i];
            float bandEnd;
            if(i != audioBands.Length - 1) 
            { 
                bandEnd = audioBands[i+1] - 1; 
            } else { 
                bandEnd = 1023f; 
            }
            _lut[i] = bandStart;
...
[CustomEditor(typeof(AudioLink2))]
public class AudioLinkEditor2 : Editor
{
