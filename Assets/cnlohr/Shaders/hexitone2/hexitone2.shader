Shader "cnlohr/hexitone2"
{
	Properties
	{
		...
		_PositionControlOffset( "Position Control Offset", Vector ) = (0,0,0,0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			float BRRFalloff( float2 xyloc ) {
				...
				{				
					float4 BodyPos = PlayerPositions[i] - _PositionControlOffset;
				...
				return saturate( intensity  + 0.115 );
			#else
				return .8;
			#endif
			}
...
#if _is_floormode_local
				const uint modes = 9;
				float mixtime = gtime/200.;
				
				//mixtime = 8;
				uint mode = ((uint)floor( mixtime ))%modes;
				float modemix = 1/(exp((-frac(mixtime)+0.5)*120)+1);
