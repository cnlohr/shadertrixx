    ...
    public class ReactiveFloorAudioLink : UdonSharpBehaviour
    {
        ...
        private void Start()
        {
            for(int i = 0; i < renderer.sharedMaterials.Length;i++) {
                if(renderer.sharedMaterials[i].name.StartsWith(material.name))
                {
                    _targetMaterial = renderer.sharedMaterials[i];
                    break;
                }
            }
        ...
        private void Update()
        {
            ...
            for(int i = 0; i < _numPlayers; i++)
            {
				...
				Debug.Log( "Pos: " + _positionArray[i].x + " " + _positionArray[i].y + " " + _positionArray[i].z + " " + _targetMaterial.name );
            }
