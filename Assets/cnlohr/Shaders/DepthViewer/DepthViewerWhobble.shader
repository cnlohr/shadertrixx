Shader "cnlohr/DepthViewerWhobble"
{
    Properties
    {
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				float lineism = 0;
				float4 grab = tex2D( _GrabTexture, screenUV.xy );
				if( depth < 999 )
				...
				float centerlineism = saturate(1.-abs( .5-lineism )*2);
				col.rgb = deldep.xxx*100;
				lineism += centerlineism;
				col.rgb += centerlineism;
				
				col.a = saturate(lineism);
